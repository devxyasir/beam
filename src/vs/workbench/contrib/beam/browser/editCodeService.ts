...
import { BEAM_ACCEPT_DIFF_ACTION_ID, BEAM_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { FeatureName } from '../common/beamSettingsTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IBeamSettingsService private readonly _settingsService: IBeamSettingsService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._beamModelService.initializeModel(model.uri)
	...
	// 		message: `Beam Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'void.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Beam's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(BEAM_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/beameditor/void/issues/new) it.` : undefined
	// 	})
	// }
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'beam-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'beam-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'beam-highlightBG')
	...
	private _getCurrentVoidFileSnapshot = (uri: URI): BeamFileSnapshot => {
		const { model } = this._beamModelService.getModel(uri)
	...
	private _restoreVoidFileSnapshot = async (uri: URI, snapshot: BeamFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: BeamFileSnapshot = this._getCurrentVoidFileSnapshot(uri)
		let afterSnapshot: BeamFileSnapshot | null = null
		...
		const onFinishEdit = async () => {
			...
			await this._beamModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}
	...
	public restoreVoidFileSnapshot(uri: URI, snapshot: BeamFileSnapshot): void {
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Beam: diff.type not recognized on: ${lastDiff}`)
		}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Beam: diff.type not recognized on: ${from}`)
		}
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Beam: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Beam error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IBeamCommandBarService private readonly _voidCommandBarService: IBeamCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(BEAM_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(BEAM_REJECT_DIFF_ACTION_ID);
