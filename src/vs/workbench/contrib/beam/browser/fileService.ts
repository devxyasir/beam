...
class FilePromptActionService extends Action2 {
	private static readonly BEAM_COPY_FILE_PROMPT_ID = 'void.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.BEAM_COPY_FILE_PROMPT_ID,
			title: localize2('voidCopyPrompt', 'Beam: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await beamModelService.getModelSafe(uri)).model?.getLanguageId() || '',
