...
export class MetricsMainService extends Disposable implements IMetricsService {
	...
	// helper - looks like this is stored in a .vscdb file in ~/Library/Application Support/Beam
	...
	private get oldId() {
		...
		const newKey = 'beam.app.oldMachineId'
		...
		const oldValue = this._appStorage.get('beam.machineId', StorageScope.APPLICATION) ?? 'NULL' // the old way of getting the key
		...
		// private get oldId() {
		// 	return this._memoStorage('beam.app.oldMachineId', StorageTarget.MACHINE, 'NULL')
		// }
	}
	...
	private get distinctId() {
		...
		return this._memoStorage('beam.app.machineId', StorageTarget.MACHINE, setValIfNotExist)
	}

	// just to see if there are ever multiple machineIDs per userID (instead of this, we should just track by the user's email)
	private get userId() {
		return this._memoStorage('beam.app.userMachineId', StorageTarget.USER)
	}
	...
	async initialize() {
		...
		const { commit, version, beamVersion, beamRelease, quality } = this._productService
		...
		console.log('User is opted out of basic Beam metrics?', didOptOut)
		if (didOptOut) {
			this.client.optOut()
		}
		else {
		...
		console.log('Beam posthog metrics info:', JSON.stringify(identifyMessage, null, 2))
	}
