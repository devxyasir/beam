...
export const BeamInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
	...
	return <>
		...
		{isMenuOpen && (
			...
			>
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="px-2 py-1 text-beam-fg-1 bg-beam-bg-2-alt border-b border-beam-border-3 sticky top-0 bg-beam-bg-1 z-10 select-none pointer-events-none">
					{optionText ?
				...
				<div className='max-h-[400px] w-full max-w-full overflow-y-auto overflow-x-auto'>
					<div className="w-max min-w-full flex flex-col gap-0 text-nowrap flex-nowrap">
						{options.length === 0 ?
							<div className="text-beam-fg-3 px-3 py-0.5">No results found</div>
							: options.map((o, oIdx) => {
...
export const BeamSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps }: {
	...
	return (
		<input
			...
			placeholder={placeholder}
			disabled={disabled}
			className={`w-full resize-none bg-beam-bg-1 text-beam-fg-1 placeholder:text-beam-fg-3 border border-beam-border-2 focus:border-beam-border-1
...
export const BeamInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline }: {
	...
	return <WidgetComponent
		className='
			bg-beam-bg-1
			@@beam-force-child-placeholder-beam-fg-1
		'
		ctor={InputBox}
				...
				<div className="relative w-full">
					...
					<div
						className={`relative ${size === 'xxs' ? 'h-0.5' :
							size === 'xs' ? 'h-1' :
								size === 'sm' ? 'h-1.5' :
									size === 'sm+' ? 'h-2' : 'h-2.5'
							} bg-beam-bg-2 rounded-full cursor-pointer`}
						onClick={handleTrackClick}
					>
					...
					<div
						className={`absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2
							...
							bg-beam-fg-1 rounded-full shadow-md ${disabled ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}
							border border-beam-fg-1`}
...
export const _BeamSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className }: {
	...
	return <WidgetComponent
		className={`
			...
			@@[&_select]:!beam-text-beam-fg-3
			@@[&_select]:!beam-text-xs
			!text-beam-fg-3
			${className ?? ''}
		`}
		ctor={SelectBox}
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	return <div ref={divRef} className='relative z-0 px-2 py-1 bg-beam-bg-3'>
		<WidgetComponent
...
const InlineDiffLines = ({ lines, kind }: { lines: string[], kind: 'added' | 'removed' }) => {
	const isAdded = kind === 'added';
	const prefix = isAdded ? '+' : '-';

	return <div className={isAdded ? '@@beam-inline-diff-added' : '@@beam-inline-diff-removed'}>
		{lines.map((line, index) => (
			<div key={`${kind}-${index}`} className="grid min-h-[18px] leading-[18px]" style={{ gridTemplateColumns: '2rem minmax(0, 1fr)' }}>
				<span className="select-none text-center opacity-80">{prefix}</span>
...
const InlineDiffBlock = ({ block }: { block: ExtractedSearchReplaceBlock }) => {
	const removedLines = splitDiffLines(block.orig);
	const addedLines = splitDiffLines(block.final);

	return (
		<div className="w-full overflow-hidden rounded-md border border-beam-border-2 bg-beam-bg-3 font-mono text-[11px] text-beam-fg-1 shadow-sm @@beam-inline-diff">
			{removedLines.length > 0 && <InlineDiffLines lines={removedLines} kind="removed" />}
			{addedLines.length > 0 && <InlineDiffLines lines={addedLines} kind="added" />}
		</div>
	);
};
...
/**
 * ToolDiffEditor shows an inline preview between original and modified code blocks.
 * Props:
...
export const BeamDiffEditor = ({ uri, searchReplaceBlocks, language }: { uri?: any, searchReplaceBlocks: string, language?: string }) => {
	...
	if (blocks.length === 0) {
		return <div className="w-full p-4 text-beam-fg-4 text-sm">No changes found</div>;
	}
		...
		<div className="w-full flex flex-col gap-2">
			{blocks.map((block, index) => (
				<div key={index} className="w-full">
					{blocks.length > 1 && (
						<div className="text-beam-fg-4 text-xs mb-1 px-1">
							Change {index + 1} of {blocks.length}
						</div>
					)}
					<InlineDiffBlock block={block} />
				</div>
			))}
		</div>
	);
