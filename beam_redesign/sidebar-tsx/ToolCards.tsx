...
export const ToolHeaderWrapper = ({
	...
	const desc1HTML = <span
		className={`text-[#43434a] text-xs italic truncate ml-2
	...
	return (<div className='@@beam-tool-enter'>
		<div className={`w-full border border-[rgba(255,255,255,0.06)] rounded-lg px-2.5 py-1.5 bg-[#17171a] overflow-hidden @@beam-tool-card ${className}`}>
			{/* header */}
			<div className={`select-none flex items-center min-h-[24px]`}>
				<div className={`flex items-center w-full gap-x-2 overflow-hidden justify-between ${isRejected ? 'line-through' : ''}`}>
					...
					<div className='ml-1 flex items-center overflow-hidden'>
						...
						>
							{isDropdown && (<ChevronRight
								className={`
								text-[#43434a] mr-0.5 h-3.5 w-3.5 flex-shrink-0 transition-transform duration-100 ease-[cubic-bezier(0.4,0,0.2,1)]
								${isExpanded ? 'rotate-90' : ''}
							`}
							/>)}
							<span className="text-[#9b9ba8] text-xs flex-shrink-0">{title}</span>
					...
					<div className="flex items-center gap-x-2 flex-shrink-0">
						...
						/>}
						{desc2 && <span className="text-[#43434a] text-xs" onClick={desc2OnClick}>{desc2}</span>}
						{numResults !== undefined && (
							<span className="text-[#43434a] text-xs ml-auto mr-1">
			...
			{<div
				className={`overflow-hidden transition-all duration-200 ease-in-out @@beam-tool-expand ${isExpanded ? 'opacity-100 py-1 scale-y-100' : 'max-h-0 opacity-0 scale-y-95'}
					text-[#43434a] rounded-sm overflow-x-auto
				  `}
			>
				{children}
			</div>}
...
export const ToolActivityRow = ({
	...
	return <div className={`@@beam-tool-enter @@beam-activity-row ${isRejected ? 'opacity-60 line-through' : ''}`}>
		...
		>
			...
			<span className='text-[#9b9ba8] flex-shrink-0'>{verb}</span>
...
export const TerminalToolCard = ({
	...
	return <div className={`@@beam-terminal-card @@beam-tool-enter ${isRejected ? 'opacity-60 line-through' : ''}`}>
		<button
			type='button'
			className='flex w-full items-center justify-between gap-3 border-b border-[rgba(255,255,255,0.05)] px-3 py-2 text-left'
			onClick={() => setIsOpen(v => !v)}
		>
			<div className='flex min-w-0 items-center gap-2'>
				<ChevronRight size={14} className={`flex-shrink-0 text-[#43434a] transition-transform duration-100 ${isOpen ? 'rotate-90' : ''}`} />
				<Terminal size={14} className='flex-shrink-0 text-[color:var(--beam-tool-terminal)]' />
				<span className='truncate text-xs text-[#65656e]'>Command {commandLabel}</span>
				{terminalName && <span className='truncate text-xs text-[#43434a]'>{terminalName}</span>}
			</div>
		...
		{isOpen && <div className='overflow-hidden'>
			<div className='@@beam-terminal-command px-3 py-2 font-mono text-xs leading-5'>
				<span className={isError ? 'text-red-400' : 'text-emerald-400'}>âŠ™</span>
