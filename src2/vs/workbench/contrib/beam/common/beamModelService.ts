...
export interface IBeamModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): BeamModelType;
	getModelFromFsPath(fsPath: string): BeamModelType;
	getModelSafe(uri: URI): Promise<BeamModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const IBeamModelService = createDecorator<IBeamModelService>('voidVoidModelService');

class BeamModelService extends Disposable implements IBeamModelService {
	...
	getModelSafe = async (uri: URI): Promise<BeamModelType> => {
...
registerSingleton(IBeamModelService, BeamModelService, InstantiationType.Eager);
