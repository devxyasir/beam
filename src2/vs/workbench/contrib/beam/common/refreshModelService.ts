...
import { RefreshableProviderName, refreshableProviderNames, SettingsOfProvider } from './beamSettingsTypes.js';
...
export class RefreshModelService extends Disposable implements IRefreshModelService {
	...
	constructor(
		...
		const initializeAutoPollingAndOnChange = () => {
			...
			if (!beamSettingsService.state.globalSettings.autoRefreshModels) return

			for (const providerName of refreshableProviderNames) {

				// const { '_didFillInProviderSettings': enabled } = this.beamSettingsService.state.settingsOfProvider[providerName]
				this.startRefreshingModels(providerName, autoOptions)

				// every time providerName.enabled changes, refresh models too, like a useEffect
				let relevantVals = () => refreshBasedOn[providerName].map(settingName => beamSettingsService.state.settingsOfProvider[providerName][settingName])
				let prevVals = relevantVals() // each iteration of a for loop has its own context and vars, so this is ok
				disposables.add(
					beamSettingsService.onDidChangeState(() => { // we might want to debounce this
		...
		beamSettingsService.waitForInitState.then(() => {
			initializeAutoPollingAndOnChange()
			this._register(
				beamSettingsService.onDidChangeState((type) => { if (typeof type === 'object' && type[1] === 'autoRefreshModels') initializeAutoPollingAndOnChange() })
			)
		})
	...
	startRefreshingModels: IRefreshModelService['startRefreshingModels'] = (providerName, options) => {
		...
		const autoPoll = () => {
			if (this.beamSettingsService.state.globalSettings.autoRefreshModels) {
				// resume auto-polling
		...
		listFn({
			...
			onSuccess: ({ models }) => {
				...
				this.beamSettingsService.setAutodetectedModels(
					providerName,
					models.map(model => {
				...
				if (options.enableProviderOnSuccess) this.beamSettingsService.setSettingOfProvider(providerName, '_didFillInProviderSettings', true)
