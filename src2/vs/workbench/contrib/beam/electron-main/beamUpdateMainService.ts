...
export class BeamMainUpdateService extends Disposable implements IBeamUpdateService {
	...
	async check(explicit: boolean): Promise<BeamCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Beam to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<BeamCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/devxyasir/beam/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Beam is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Beam is up-to-date!'
					}
				}
				else {
