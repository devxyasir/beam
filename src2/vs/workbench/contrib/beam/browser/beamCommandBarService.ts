...
import { mountBeamCommandBar } from './react/out/beam-editor-widgets-tsx/index.js'
...
import { BEAM_ACCEPT_DIFF_ACTION_ID, BEAM_REJECT_DIFF_ACTION_ID, BEAM_GOTO_NEXT_DIFF_ACTION_ID, BEAM_GOTO_PREV_DIFF_ACTION_ID, BEAM_GOTO_NEXT_URI_ACTION_ID, BEAM_GOTO_PREV_URI_ACTION_ID, BEAM_ACCEPT_FILE_ACTION_ID, BEAM_REJECT_FILE_ACTION_ID, BEAM_ACCEPT_ALL_DIFFS_ACTION_ID, BEAM_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IBeamCommandBarService {
...
export const IBeamCommandBarService = createDecorator<IBeamCommandBarService>('BeamCommandBarService');
...
export class BeamCommandBarService extends Disposable implements IBeamCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'void.BeamCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._beamModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IBeamCommandBarService, BeamCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type BeamCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountBeamCommandBar(root, accessor, { uri, editor } satisfies BeamCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies BeamCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidAcceptDiffAction', 'Beam: Accept Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidRejectDiffAction', 'Beam: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidGoToNextDiffAction', 'Beam: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('voidGoToPrevDiffAction', 'Beam: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('voidGoToNextUriAction', 'Beam: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('voidGoToPrevUriAction', 'Beam: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('voidAcceptFileAction', 'Beam: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('voidRejectFileAction', 'Beam: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('voidAcceptAllDiffsAction', 'Beam: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: BEAM_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('voidRejectAllDiffsAction', 'Beam: Reject All Diffs in All Files'),
			keybinding: {
