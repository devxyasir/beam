...
import { isFeatureNameDisabled } from '../../../../common/beamSettingsTypes.js';
...
export const IconShell1 = ({ onClick, Icon, disabled, className, ...props }: IconButtonProps & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  ...
  return <button
    ...
    // border border-beam-border-1 rounded
    className={` beam-size-[18px] beam-p-[2px] beam-flex beam-items-center beam-justify-center beam-text-sm beam-text-beam-fg-3 hover:beam-brightness-110 disabled:beam-opacity-50 disabled:beam-cursor-not-allowed ${
...
export const useApplyStreamState = ({ applyBoxId }: {applyBoxId: string;}) => {
  const accessor = useAccessor();
  const voidCommandBarService = accessor.get('IBeamCommandBarService');
...
export const StatusIndicator = ({ indicatorColor, title, className, ...props }: {indicatorColor: IndicatorColor;title?: React.ReactNode;className?: string;} & React.HTMLAttributes<HTMLDivElement>) => {
  return (
    <div className={`beam-flex beam-flex-row beam-text-beam-fg-3 beam-text-xs beam-items-center beam-gap-1.5 ${className}`} {...props}>
			{title && <span className="beam-opacity-80">{title}</span>}
			<div
        className={` beam-size-1.5 beam-rounded-full beam-border ${
        indicatorColor === 'dark' ? "beam-bg-[rgba(0,0,0,0)] beam-border-beam-border-1" :
        indicatorColor === 'orange' ? "beam-bg-orange-500 beam-border-orange-500 beam-shadow-[0_0_4px_0px_rgba(234,88,12,0.6)]" :
        indicatorColor === 'green' ? "beam-bg-green-500 beam-border-green-500 beam-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" :
        indicatorColor === 'yellow' ? "beam-bg-yellow-500 beam-border-yellow-500 beam-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" : "beam-bg-beam-border-1 beam-border-beam-border-1"} `} />
...
const tooltipPropsForApplyBlock = ({ tooltipName, color = undefined, position = 'top', offset = undefined }: {tooltipName: string;color?: IndicatorColor;position?: PlacesType;offset?: number;}) => ({
  'data-tooltip-id': color === 'orange' ? `beam-tooltip-orange` : color === 'green' ? 'beam-tooltip-green' : 'beam-tooltip',
...
const ApplyButtonsForEdit = ({
  ...
  const onClickSubmit = useCallback(async () => {
    ...
    if (!applyDonePromise) {
      notificationService.info(`Beam Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`);
    }
    ...
    applyDonePromise?.catch((e) => {
      ...
      notificationService.info(`Beam Error: There was a problem running Apply: ${e}.`);
...
export const BlockCodeApplyWrapper = ({
  ...
  <ListableToolItem
    name={<span className="beam-not-italic">{getBasename(uri.fsPath)}</span>}
    isSmall={true}
    showDot={false}
  ...
  return <div className="beam-border beam-border-beam-border-3 beam-rounded beam-overflow-hidden beam-bg-beam-bg-3 beam-my-1">
		{/* header */}
		<div className=" beam-select-none beam-flex beam-justify-between beam-items-center beam-py-1 beam-px-2 beam-border-b beam-border-beam-border-3 beam-cursor-default">
			<div className="beam-flex beam-items-center">
				<StatusIndicatorForApplyButton uri={uri} applyBoxId={applyBoxId} />
				<span className="beam-text-[13px] beam-font-light beam-text-beam-fg-3">
					{name}
				</span>
			</div>
			<div className={`${canApply ? "" : "beam-hidden"} beam-flex beam-items-center beam-gap-1`}>
