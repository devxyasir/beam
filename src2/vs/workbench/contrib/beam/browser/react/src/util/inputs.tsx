...
export const WidgetComponent = <CtorParams extends any[], Instance>({ ctor, propsFn, dispose, onCreateInstance, children, className
  ...
  return <div ref={containerRef} className={className === undefined ? `beam-w-full` : className}>{children}</div>;
};
...
export const BeamInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
  ...
  return <>
      ...
      className={`beam-w-full beam-resize-none beam-max-h-[500px] beam-overflow-y-auto beam-text-beam-fg-1 placeholder:beam-text-beam-fg-3 ${className}`}
      style={{
    ...
    <div
      ...
      className="beam-z-[100] beam-border-beam-border-3 beam-bg-beam-bg-2-alt beam-border beam-rounded beam-shadow-lg beam-flex beam-flex-col beam-overflow-hidden"
      style={{
        position: strategy,
        top: y ?? 0,
        left: x ?? 0,
      ...
      onWheel={(e) => e.stopPropagation()}>
				...
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="beam-px-2 beam-py-1 beam-text-beam-fg-1 beam-bg-beam-bg-2-alt beam-border-b beam-border-beam-border-3 beam-sticky beam-top-0 beam-bg-beam-bg-1 beam-z-10 beam-select-none beam-pointer-events-none">
					{optionText ?
        ...
        <div className="beam-opacity-50">Enter text to filter...</div>
        }
				</div>}


				{/* Options list */}
				<div className="beam-max-h-[400px] beam-w-full beam-max-w-full beam-overflow-y-auto beam-overflow-x-auto">
					<div className="beam-w-max beam-min-w-full beam-flex beam-flex-col beam-gap-0 beam-text-nowrap beam-flex-nowrap">
						{options.length === 0 ?
          <div className="beam-text-beam-fg-3 beam-px-3 beam-py-0.5">No results found</div> :
          options.map((o, oIdx) => {
            ...
            return (
              ...
              <div
                ...
                key={o.fullName}
                className={` beam-flex beam-items-center beam-gap-2 beam-px-3 beam-py-1 beam-cursor-pointer ${


                oIdx === optionIdx ? "beam-bg-blue-500 beam-text-white/80" : "beam-bg-beam-bg-2-alt beam-text-beam-fg-1"} `}
                ...
                onMouseMove={() => {setOptionIdx(oIdx);}}>
										...
										{o.fullName && o.fullName !== o.abbreviatedName && <span className="beam-opacity-60 beam-text-sm">{o.fullName}</span>}
...
export const BeamSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps
  ...
  return (
    <input
      ...
      placeholder={placeholder}
      disabled={disabled}
      className={`beam-w-full beam-resize-none beam-bg-beam-bg-1 beam-text-beam-fg-1 placeholder:beam-text-beam-fg-3 beam-border beam-border-beam-border-2 focus:beam-border-beam-border-1 ${
      compact ? "beam-py-1 beam-px-2" : "beam-py-2 beam-px-4 "} beam-rounded ${

      disabled ? "beam-opacity-50 beam-cursor-not-allowed" : ""} ${
      className}`}
      style={{
...
export const BeamInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline
  ...
  return <WidgetComponent
    className=" beam-bg-beam-bg-1 beam-force-child-placeholder-beam-fg-1 "
...
export const BeamSlider = ({
  ...
  return (
    <div className={`beam-inline-flex beam-items-center beam-flex-shrink-0 ${className}`}>
			{/* Outer container with padding to account for thumb overhang */}
			<div className={`beam-relative beam-flex-shrink-0 ${disabled ? "beam-opacity-25" : ""}`}
      style={{
        width
        ...
        // paddingRight: thumbSizePx / 2
				...
				<div className="beam-relative beam-w-full">
					...
					<div
            className="beam-absolute beam-w-full beam-cursor-pointer"
            style={{
              height: '16px',
              top: '50%',
            ...
            onClick={handleTrackClick} />
					...
					{/* Track */}
					<div
            className={`beam-relative ${size === 'xxs' ? "beam-h-0.5" :
            size === 'xs' ? "beam-h-1" :
            size === 'sm' ? "beam-h-1.5" :
            size === 'sm+' ? "beam-h-2" : "beam-h-2.5"} beam-bg-beam-bg-2 beam-rounded-full beam-cursor-pointer`}

            onClick={handleTrackClick}>
            
						{/* Filled part of track */}
						<div
              className={`beam-absolute beam-left-0 ${size === 'xxs' ? "beam-h-0.5" :
              size === 'xs' ? "beam-h-1" :
              size === 'sm' ? "beam-h-1.5" :
              size === 'sm+' ? "beam-h-2" : "beam-h-2.5"} beam-bg-beam-fg-1 beam-rounded-full`}
              ...
              style={{ width: `${percentage}%` }} />
					...
					{/* Thumb */}
					<div
            className={`beam-absolute beam-top-1/2 beam-transform -beam-translate-x-1/2 -beam-translate-y-1/2 ${
            size === 'xxs' ? "beam-h-2 beam-w-2" :
            size === 'xs' ? "beam-h-2.5 beam-w-2.5" :
            size === 'sm' ? "beam-h-3 beam-w-3" :
            size === 'sm+' ? "beam-h-3.5 beam-w-3.5" : "beam-h-4 beam-w-4"} beam-bg-beam-fg-1 beam-rounded-full beam-shadow-md ${

            disabled ? "beam-cursor-not-allowed" : "beam-cursor-grab active:beam-cursor-grabbing"} beam-border beam-border-beam-fg-1`}
...
export const BeamSwitch = ({
  ...
  return (
    <label className="beam-inline-flex beam-items-center" {...props}>
			<div
        onClick={() => !disabled && onChange(!value)}
        className={` beam-cursor-pointer beam-relative beam-inline-flex beam-items-center beam-rounded-full beam-transition-colors beam-duration-200 beam-ease-in-out ${


        value ? "beam-bg-zinc-900 dark:beam-bg-white" : "beam-bg-white dark:beam-bg-zinc-600"} ${
        disabled ? "beam-opacity-25" : ""} ${
        size === 'xxs' ? "beam-h-3 beam-w-5" : ""} ${
        size === 'xs' ? "beam-h-4 beam-w-7" : ""} ${
        size === 'sm' ? "beam-h-5 beam-w-9" : ""} ${
        size === 'sm+' ? "beam-h-5 beam-w-10" : ""} ${
        size === 'md' ? "beam-h-6 beam-w-11" : ""} `}>

        
				<span
          className={` beam-inline-block beam-transform beam-rounded-full beam-bg-white dark:beam-bg-zinc-900 beam-shadow beam-transition-transform beam-duration-200 beam-ease-in-out ${

          size === 'xxs' ? "beam-h-2 beam-w-2" : ""} ${
          size === 'xs' ? "beam-h-2.5 beam-w-2.5" : ""} ${
          size === 'sm' ? "beam-h-3 beam-w-3" : ""} ${
          size === 'sm+' ? "beam-h-3.5 beam-w-3.5" : ""} ${
          size === 'md' ? "beam-h-4 beam-w-4" : ""} ${
          size === 'xxs' ? value ? "beam-translate-x-2.5" : "beam-translate-x-0.5" : ""} ${
          size === 'xs' ? value ? "beam-translate-x-3.5" : "beam-translate-x-0.5" : ""} ${
          size === 'sm' ? value ? "beam-translate-x-5" : "beam-translate-x-1" : ""} ${
          size === 'sm+' ? value ? "beam-translate-x-6" : "beam-translate-x-1" : ""} ${
          size === 'md' ? value ? "beam-translate-x-6" : "beam-translate-x-1" : ""} `} />
...
export const BeamCheckBox = ({ label, value, onClick, className }: {label: string;value: boolean;onClick: (checked: boolean) => void;className?: string;}) => {
...
export const BeamCustomDropdownBox = <T extends NonNullable<any>,>({
  options,
  selectedOption,
  onChangeOption,
  ...
  return (
    <div className={`beam-inline-block beam-relative ${className}`}>
			{/* Hidden measurement div */}
			<div
        ref={measureRef}
        className="beam-opacity-0 beam-pointer-events-none beam-absolute -beam-left-[999999px] -beam-top-[999999px] beam-flex beam-flex-col"
        aria-hidden="true">
          ...
          return (
            <div key={optionName + optionDetail} className="beam-flex beam-items-center beam-whitespace-nowrap">
							<div className="beam-w-4" />
							<span className="beam-flex beam-justify-between beam-w-full">
        ...
        className="beam-flex beam-items-center beam-h-4 beam-bg-transparent beam-whitespace-nowrap hover:beam-brightness-90 beam-w-full"
        onClick={() => setIsOpen(!isOpen)}>
        
				<span className={`beam-truncate ${arrowTouchesText ? "beam-mr-1" : ""}`}>
					{getOptionDisplayName(selectedOption)}
				</span>
				<svg
          className={`beam-size-3 beam-flex-shrink-0 ${arrowTouchesText ? "" : "beam-ml-auto"}`}
          viewBox="0 0 12 12"
          fill="none">
      ...
      <div
        ref={refs.setFloating}
        className="beam-z-[100] beam-bg-beam-bg-1 beam-border-beam-border-3 beam-border beam-rounded beam-shadow-lg"
        style={{
          position: strategy,
          top: y ?? 0,
          left: x ?? 0,
        ...
        onWheel={(e) => e.stopPropagation()}>
        <div className="beam-overflow-auto beam-max-h-80">
            ...
            return (
              <div
                key={optionName}
                className={`beam-flex beam-items-center beam-px-2 beam-py-1 beam-pr-4 beam-cursor-pointer beam-whitespace-nowrap beam-transition-all beam-duration-100 ${

                thisOptionIsSelected ? "beam-bg-blue-500 beam-text-white/80" : "hover:beam-bg-blue-500 hover:beam-text-white/80"} `}
                ...
                onClick={() => {
                  ...
                  setIsOpen(false);
									...
									<div className="beam-w-4 beam-flex beam-justify-center beam-flex-shrink-0">
										{thisOptionIsSelected &&
                  <svg className="beam-size-3" viewBox="0 0 12 12" fill="none">
												<path
                      d="M10 3L4.5 8.5L2 6"
                      stroke="currentColor"
                      strokeWidth="1.5"
                      ...
                      strokeLinejoin="round" />
									...
									<span className="beam-flex beam-justify-between beam-items-center beam-w-full beam-gap-x-1">
										<span>{optionName}</span>
										<span className="beam-opacity-60">{optionDetail}</span>
									</span>
								</div>);
...
export const _BeamSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className
  ...
  return <WidgetComponent
    className={` select-child-restyle [&_select]:!beam-text-beam-fg-3 [&_select]:!beam-text-xs !beam-text-beam-fg-3 ${
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
  ...
  return <div ref={divRef} className="beam-relative beam-z-0 beam-px-2 beam-py-1 beam-bg-beam-bg-3">
		<WidgetComponent
...
export const BeamButtonBgDarken = ({ children, disabled, onClick, className }: {children: React.ReactNode;disabled?: boolean;onClick: () => void;className?: string;}) => {
  ...
  className={`beam-px-3 beam-py-1 beam-bg-black/10 dark:beam-bg-white/10 beam-rounded-sm beam-overflow-hidden beam-whitespace-nowrap beam-flex beam-items-center beam-justify-center ${className || ''}`}
  onClick={onClick}>
    {children}</button>;
};

// export const BeamScrollableElt = ({ options, children }: { options: ScrollableElementCreationOptions, children: React.ReactNode }) => {
...
// export const BeamSelectBox = <T,>({ onChangeSelection, initVal, selectBoxRef, options }: {
// 	initVal: T;
...
// export const BeamCheckBox = ({ onChangeChecked, initVal, label, checkboxRef, }: {
...
const SingleDiffEditor = ({ block, lang }: {block: ExtractedSearchReplaceBlock;lang: string | undefined;}) => {
  ...
  return (
    <div className="beam-w-full beam-bg-beam-bg-3 bg-editor-style-override" ref={divRef} />);
 ...
 */
export const BeamDiffEditor = ({ uri, searchReplaceBlocks, language }: {uri?: any;searchReplaceBlocks: string;language?: string;}) => {
  const accessor = useAccessor();
  const languageService = accessor.get('ILanguageService');
  ...
  if (blocks.length === 0) {
    return <div className="beam-w-full beam-p-4 beam-text-beam-fg-4 beam-text-sm">No changes found</div>;
  }

  // Display all blocks
  return (
    <div className="beam-w-full beam-flex beam-flex-col beam-gap-2">
			{blocks.map((block, index) =>
      <div key={index} className="beam-w-full">
					{blocks.length > 1 &&
        <div className="beam-text-beam-fg-4 beam-text-xs beam-mb-1 beam-px-1">
