...
const LatexRender = ({ latex }: {latex: string;}) => {
  return <span className="beam-katex-error beam-text-red-500">{latex}</span>;
  // try {
  // 	let formula = latex;
...
const Codespan = ({ text, className, onClick, tooltip }: {text: string;className?: string;onClick?: () => void;tooltip?: string;}) => {
  ...
  return <code
    className={`beam-font-mono beam-font-medium beam-rounded-sm beam-bg-beam-bg-1 beam-px-1 ${className}`}
    onClick={onClick}
    {...tooltip ? {
...
const CodespanWithLink = ({ text, rawText, chatMessageLocation }: {text: string;rawText: string;chatMessageLocation: ChatMessageLocation;}) => {
  ...
  return <Codespan
    text={displayText}
    onClick={onClick}
    className={link ? "beam-underline hover:beam-brightness-90 beam-transition-all beam-duration-200 beam-cursor-pointer" : ""}
...
const RenderToken = ({ token, inPTag, codeURI, chatMessageLocation, tokenIdx, ...options }: {token: Token | string;inPTag?: boolean;codeURI?: URI;chatMessageLocation?: ChatMessageLocation;tokenIdx: string;} & RenderTokenOptions): React.ReactNode => {
  ...
  if (t.type === 'table') {
    ...
    // return (
    // 	<div>
    // 		<table className={'min-w-full border border-beam-bg-2'}>
    // 			<thead>
    // 				<tr className='bg-beam-bg-1'>
    // 					{t.header.map((cell: any, index: number) => (
    // 						<th
    // 							key={index}
    // 							className='px-4 py-2 border border-beam-bg-2 font-semibold'
    ...
    // 					<tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-beam-bg-1'}>
    // 						{row.map((cell: any, cellIndex: number) => (
    // 							<td
    // 								key={cellIndex}
    // 								className={'px-4 py-2 border border-beam-bg-2'}
  ...
  if (t.type === 'paragraph') {
    ...
    if (latexSegments.length !== 0) {
      if (inPTag) {
        return <span className="beam-block">{latexSegments}</span>;
      }
    ...
    if (inPTag) return <span className="beam-block">{contents}</span>;
    return <p>{contents}</p>;
  }
  ...
  if (t.type === 'link') {
    return (
      <a
        ...
        className="beam-underline beam-cursor-pointer hover:beam-brightness-90 beam-transition-all beam-duration-200 beam-text-beam-fg-2">
  ...
  // default
  return (
    <div className="beam-bg-orange-50 beam-rounded-sm beam-overflow-hidden beam-p-2">
			<span className="beam-text-sm beam-text-orange-500">Unknown token rendered...</span>
		</div>);
