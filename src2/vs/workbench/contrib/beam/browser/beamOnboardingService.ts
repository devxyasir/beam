...
import { mountBeamOnboarding } from './react/out/beam-onboarding/index.js'
...
export class OnboardingContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.beamOnboarding';
	...
	private initialize(): void {
		...
		if (workbench) {
			...
			const onboardingContainer = h('div.beam-onboarding-container').root;
			workbench.appendChild(onboardingContainer);
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountBeamOnboarding(onboardingContainer, accessor);
