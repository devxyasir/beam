...
/* eslint-disable */ // Beam
import { BEAM_CTRL_K_ACTION_ID, BEAM_CTRL_L_ACTION_ID } from '../../../contrib/beam/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		const update = async () => {
			...
			// Beam - if the workbench is empty, show open
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				openFolderButton.root.classList.add('beam-openfolder-button')
				...
				openSSHButton.root.classList.add('beam-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					voidIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('beam-link')
				...
				// show them Beam keybindings
				const keys = this.keybindingService.lookupKeybinding(BEAM_CTRL_L_ACTION_ID);
				...
				const keys2 = this.keybindingService.lookupKeybinding(BEAM_CTRL_K_ACTION_ID);
