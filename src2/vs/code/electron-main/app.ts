...
import { IBeamUpdateService } from '../../workbench/contrib/beam/common/beamUpdateService.js';
import { MetricsMainService } from '../../workbench/contrib/beam/electron-main/metricsMainService.js';
import { BeamMainUpdateService } from '../../workbench/contrib/beam/electron-main/beamUpdateMainService.js';
import { LLMMessageChannel } from '../../workbench/contrib/beam/electron-main/sendLLMMessageChannel.js';
import { BeamSCMService } from '../../workbench/contrib/beam/electron-main/beamSCMMainService.js';
import { IBeamSCMService } from '../../workbench/contrib/beam/common/beamSCMTypes.js';
import { MCPChannel } from '../../workbench/contrib/beam/electron-main/mcpChannel.js';
/**
...
export class CodeApplication extends Disposable {
	...
	async startup(): Promise<void> {
		this.logService.debug('Starting Beam');
		...
		// "com.microsoft.", which breaks native tabs for Beam when using this
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Beam main process services (required for services with a channel for comm between browser and electron-main (node))
		services.set(IMetricsService, new SyncDescriptor(MetricsMainService, undefined, false));
		services.set(IBeamUpdateService, new SyncDescriptor(BeamMainUpdateService, undefined, false));
		services.set(IBeamSCMService, new SyncDescriptor(BeamSCMService, undefined, false));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Beam - use loggerChannel as reference
		const metricsChannel = ProxyChannel.fromService(accessor.get(IMetricsService), disposables);
		mainProcessElectronServer.registerChannel('beam-channel-metrics', metricsChannel);

		const voidUpdatesChannel = ProxyChannel.fromService(accessor.get(IBeamUpdateService), disposables);
		mainProcessElectronServer.registerChannel('beam-channel-update', voidUpdatesChannel);

		const sendLLMMessageChannel = new LLMMessageChannel(accessor.get(IMetricsService));
		mainProcessElectronServer.registerChannel('beam-channel-llmMessage', sendLLMMessageChannel);

		// Beam added this
		const voidSCMChannel = ProxyChannel.fromService(accessor.get(IBeamSCMService), disposables);
		mainProcessElectronServer.registerChannel('beam-channel-scm', voidSCMChannel);

		// Beam added this
		const mcpChannel = new MCPChannel();
		mainProcessElectronServer.registerChannel('beam-channel-mcp', mcpChannel);
